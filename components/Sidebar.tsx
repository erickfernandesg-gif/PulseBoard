...
export function Sidebar({ userProfile, boards = [] }: SidebarProps) {
  ...
  if (!isMounted) return <div className="hidden md:flex h-full w-64 bg-white border-r border-slate-200"></div>;

  return (
    <div className={cn(
      "hidden md:flex h-full flex-col bg-white border-r border-slate-200 transition-all duration-300 relative z-20",
      isCollapsed ? "w-20" : "w-64"
    )}>
      {/* Botão de Toggle */}
      <button 
        onClick={toggleCollapse}
        className="absolute -right-3 top-6 flex h-6 w-6 items-center justify-center rounded-full bg-white border border-slate-200 text-slate-400 hover:text-slate-900 hover:bg-slate-50 transition-all shadow-sm z-30 hover:scale-110"
      >
        {isCollapsed ? <ChevronRight size={14} /> : <ChevronLeft size={14} />}
      </button>

      {/* Header Logo */}
      <div className={cn("flex h-16 shrink-0 items-center border-b border-slate-100 transition-all", isCollapsed ? "justify-center px-0" : "px-6")}>
        <div className="flex items-center gap-3">
          ...
          {!isCollapsed && <span className="text-lg font-bold text-slate-900 tracking-tight truncate">PulseBoard</span>}
        </div>
      </div>

      {/* Menu Principal (Fixado no topo) */}
      <div className={cn("py-6 space-y-1 shrink-0", isCollapsed ? "px-2" : "px-4")}>
        {!isCollapsed && <p className="px-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3">Menu Principal</p>}
        {visibleLinks.map((link) => {
          ...
          return (
            ...
            >
              ...
              <Icon size={isCollapsed ? 20 : 18} className={cn("shrink-0 transition-transform group-hover:scale-110", isActive ? "text-indigo-600" : "text-slate-400")} />
      ...
      <div className={cn("flex-1 overflow-y-auto custom-scrollbar space-y-1 pb-4", isCollapsed ? "px-2" : "px-4")}>
        {!isCollapsed ? (
          <p className="px-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3 sticky top-0 bg-white py-1 z-10">Meus Quadros</p>
        ) : (
          <div className="w-full flex justify-center mb-4"><div className="w-6 h-px bg-slate-100"></div></div>
        )}
        
        {boards?.length === 0 ? (
          !isCollapsed && <p className="px-4 py-2 text-xs text-slate-400 italic">Nenhum quadro ativo.</p>
        ) : (
          boards?.map((board) => {
            ...
            return (
              ...
              >
                {isActive && !isCollapsed && <span className="absolute left-0 top-1/2 -translate-y-1/2 h-1/2 w-1 bg-slate-300 rounded-r-full" />}
                
                {isCollapsed ? (
                  <div className={cn(
                    "flex h-7 w-7 items-center justify-center rounded-md text-[11px] font-bold transition-all",
                    isActive ? "bg-indigo-600 text-white shadow-sm" : "bg-slate-50 border border-slate-100 text-slate-400 group-hover:bg-slate-100 group-hover:text-slate-900"
                  )}>
                    {firstLetter}
                  </div>
                ) : (
                  <>
                    <div className={cn("h-2 w-2 shrink-0 rounded-full transition-colors", isActive ? "bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]" : "bg-slate-200 group-hover:bg-slate-400")} />
      ...
      {/* Footer: User & Logout */}
      <div className={cn("border-t border-slate-100 p-4 shrink-0 transition-all bg-white", isCollapsed ? "flex flex-col items-center gap-3" : "")}>
        <div className={cn(
          "flex items-center rounded-xl transition-all",
          isCollapsed ? "p-0 justify-center" : "bg-slate-50 border border-slate-100 gap-3 p-3 mb-2"
        )}>
          ...
          {!isCollapsed && (
            <div className="flex flex-col flex-1 min-w-0">
              <span className="text-sm font-bold text-slate-900 truncate">{userProfile?.full_name || "Usuário"}</span>
        ...
        <button
          ...
          className={cn(
            "flex items-center font-medium text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600 rounded-lg",
